...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, error = FALSE, eval = TRUE, results = "hide", fig.height = 16, fig.width = 16)
```
...
A couple of things here. First of all, there are a lot of retweets here. Of the `r length(um_tweets_all)` Tweets originally analysed, only `r length(um_tweets_trim)` remain after removing the retweets. This means that `r 100*(1-(length(um_tweets_trim)/length(um_tweets_all)))`% of these Tweets were retweets. Man, we ultrarunners aren't very original are we? Unfortunately this therefore drastically reduces the number of Tweets that I am analysing. Secondly, because of the way the pattern matching is done we end up with a lot of Tweets with "ultra" or "running" in them that don't match any of the three specific terms that I am looking at here. Also, this counting may be double counting some Tweets if both versions are used in a single Tweet. But I can't be bothered taking such stupidity into account right now! ;)
