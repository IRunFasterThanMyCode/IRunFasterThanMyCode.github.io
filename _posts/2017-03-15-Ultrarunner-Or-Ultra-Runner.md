...
layout: post
comments: true
title: "Ultrarunner or Ultra Runner?"
author: Sam Robson
published: true
status: publish
draft: false
...
I filled this in with details of this blog, and created a new app that allows API access for the blog. Note that to avoid errors further down the line, you need to set the "Callback URL" field to ```http://127.0.0.1:1410``` which will return to your localhost on port 1410 following authentication. Press "Create your Twitter application" to create your app. This will take you to a page with information about your new app, including a tab at the top of the page called "Keys and Access Tokens". The "API Key" and "API Secret" can then be used to access the API.
...
Since R is my go-to analysis package, I will be using the ```TwitteR``` package from Jeff Gentry to access the API. You can also access through scripting languages like perl and python, which I will likely explore in the future. But let's keep this simple. 
 
You can install ```TwitteR``` from the [Comprehensive R Archive Network](https://cran.r-project.org) by doing the following:
...
Or alternatively you can install the less stable development version from GitHub:
...
So now we are all set up and we can take a look at how to access the API. The workhorse of the ```twitteR``` package is the ```searchTwitter``` function. This can search for something like a hashtag, or key word, and can use basic boolean logic such as ```AND``` (```+```) and ```OR``` (```-```). The API actually only allows you to access information from a short time in the past, so we can't pull down data from all time, but we can get Tweets from the last week or so. So let's get the most recent Tweets relating to ultrarunning and count how many non-retweets over the last few days have used the three different terms:
...
Let's look at this code. After loading the ```twitteR``` package and specifying the number of Tweets to load, we access the Twitter API using ```searchTwitter``` and load in the 10<sup>6</sup> most recent Tweets with the terms "ultra", "running", or "ultrarunning" in them (there will be many fewer than this, but I want to capture every Tweet possible). This produces a list of 1052 objects of class ```status```, which is a specially defined reference class as a container for Twitter statuses. Next we strip out all of the retweets to leave 652 Tweets, then pull out the text from the list objects. ```sapply()``` applies the accessor function ```getText()``` to all elements of the list). Since I am only interested in three specific terms, I use regular expressions to look only at Tweets containing one of these terms (giving 360 Tweets), and then count how many Tweets contain each of the three specific terms. Finally I generate a pie chart of the results. 
 
A couple of things here. First of all, there are a lot of retweets here. Of the 1052 Tweets originally analysed, only 652 remain after removing the retweets. This means that 38.0228137% of these Tweets were retweets. Man, we ultrarunners aren't very original are we? Unfortunately this therefore drastically reduces the number of Tweets that I am analysing. Secondly, because of the way the pattern matching is done we end up with a lot of Tweets with "ultra" or "running" in them that don't match any of the three specific terms that I am looking at here. Also, this counting may be double counting some Tweets if both versions are used in a single Tweet. But I can't be bothered taking such stupidity into account right now! ;)
...
There are a lot more Tweets relating to ultra "running" compared to ultra "marathon", with 11746 Tweets in the starting data set. However, again we lose a lot of Tweets through retweets leaving us with only 2069 Tweets to play with. After trimming out Tweets that don't follow the format that I am looking at here, we are left with only 175 -- even less than in the last analysis. 
...
![plot of chunk 2017-03-15_ultramarathon_tweet_wordcloud](/figures/2017-03-15_ultramarathon_tweet_wordcloud-1.png)
